...
//  int count=0;
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    if(buf[0]=='s' && buf[1]=='u' && buf[2]=='m' && buf[3]=='a'){
                   ZeroMemory(&si,sizeof(si));
		si.cb=sizeof(si);
		ZeroMemory(&pi,sizeof(pi));
		printf("inserte el polinomio ingresando los coeficientes de cada uno, separados por un espacio\n");
		getcmd(buf, sizeof(buf));
	int n=0;
        int v[100];
        for(int i=0;sizeof(buf);i=i+2){
        	...
        	n=n+1;
                   ...
                   if(buf[0]=='r' && buf[1]=='e' && buf[2]=='s' && buf[3]=='t' && buf[3]=='a'){
                   ZeroMemory(&si,sizeof(si));
		si.cb=sizeof(si);
		ZeroMemory(&pi,sizeof(pi));
		printf("inserte el polinomio ingresando los coeficientes de cada uno, separados por un espacio\n");
